...
export abstract class SqrtPriceMath {
  ...
  public static getAmountADelta(sqrtRatioLX64: JSBI, sqrtRatioUX64: JSBI, liquidity: JSBI, roundUp: boolean): JSBI {
    ...
    return roundUp
      ? FullMath.mulDivRoundingUp(FullMath.mulDivRoundingUp(numerator1, numerator2, sqrtRatioUX64), ONE, sqrtRatioLX64)
      : JSBI.divide(JSBI.divide(JSBI.multiply(numerator1, numerator2), sqrtRatioUX64), sqrtRatioLX64)
  }

  public static getAmountBDelta(sqrtRatioLX64: JSBI, sqrtRatioUX64: JSBI, liquidity: JSBI, roundUp: boolean): JSBI {
    if (JSBI.greaterThan(sqrtRatioLX64, sqrtRatioUX64)) {
      ;[sqrtRatioLX64, sqrtRatioUX64] = [sqrtRatioUX64, sqrtRatioLX64]
    }

    return roundUp
      ? FullMath.mulDivRoundingUp(liquidity, JSBI.subtract(sqrtRatioUX64, sqrtRatioLX64), Q64)
      : JSBI.divide(JSBI.multiply(liquidity, JSBI.subtract(sqrtRatioUX64, sqrtRatioLX64)), Q64)
  }

  public static getNextSqrtPriceFromInput(sqrtPX64: JSBI, liquidity: JSBI, amountIn: JSBI, zeroForOne: boolean): JSBI {
  ...
  private static getNextSqrtPriceFromAmountARoundingUp(
    ...
    if (add) {
      ...
      return FullMath.mulDivRoundingUp(numerator1, ONE, JSBI.add(JSBI.divide(numerator1, sqrtPX64), amount))
    } else {
      let product = multiplyIn128(amount, sqrtPX64)
