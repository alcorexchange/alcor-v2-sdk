...
describe("Fraction", () => {
  describe("#quotient", () => {
    it("floor division", () => {
      expect(new Fraction(BigInt(8), BigInt(3)).quotient).toEqual(
        BigInt(2)
      ); // one below
      expect(new Fraction(BigInt(12), BigInt(4)).quotient).toEqual(
        BigInt(3)
      ); // exact
      expect(new Fraction(BigInt(16), BigInt(5)).quotient).toEqual(
        BigInt(3)
      ); // one above
    });
  });
  ...
  describe("#invert", () => {
    it("flips num and denom", () => {
      expect(
        new Fraction(BigInt(5), BigInt(10)).invert().numerator
      ).toEqual(BigInt(10));
      expect(
        new Fraction(BigInt(5), BigInt(10)).invert().denominator
      ).toEqual(BigInt(5));
    });
  });
  describe("#add", () => {
