...
describe("test", () => {
    ...
    describe("bestTradeExactOut", () => {
      ...
      const poolsData = JSON.parse(fs.readFileSync(__dirname + '/fixtures/pools.json', 'utf8'))
      const ticksData = JSON.parse(fs.readFileSync(__dirname + '/fixtures/ticks.json', 'utf8'))
      ...
      const pools = poolsData.rows.map((p) => {
        ...
        const currencyAmountIn = CurrencyAmount.fromRawAmount(parseToken(token0), 100_0000)
        const [bestExactIn] = await Trade.bestTradeExactIn(pools, currencyAmountIn, parseToken(token1));

        console.log('100.0000 TLM -> ', bestExactIn.outputAmount.toAsset(), '| priceImpact:', bestExactIn.priceImpact.toFixed())

        const currencyAmountOut = bestExactIn.outputAmount
        const [bestExactOut] = await Trade.bestTradeExactOut(pools, parseToken(token0), currencyAmountOut);

        console.log(bestExactOut.outputAmount.toAsset(), '->', bestExactOut.inputAmount.toAsset(), '| priceImpact:', bestExactOut.priceImpact.toFixed())
    });
    });
});
